    ...
    <el-card class="box-card" shadow="never">
      ...
      <div class="user-form-table mt20">
        <el-button
          ...
          @click="$router.push({name:'processManagement',query:{type:'add'}})"
        >新增</el-button>
        <el-table
          class="mt10"
          :data="tableData"
        ...
        >
          <el-table-column prop="documentNumber" width="100" label="编号"></el-table-column>
          <el-table-column prop="employeeName" label="申请人"></el-table-column>
          <el-table-column prop="applyDate" label="申请日期"></el-table-column>
          <el-table-column prop="processDirection" label="工艺方向"></el-table-column>
          <el-table-column prop="processType" label="工艺类型"></el-table-column>
          ...
          <el-table-column label="操作" align="center" width="180">
            <template v-slot="{row}">
              <common-operation
                :row="row"
                :deletUrl="deletUrl"
                :getPageList="getProcessList"
                pageUrlName="processManagement"
...
<script>
/* eslint-disable */
import { getProcessList, getProcessDelete } from "@/service/pilot";
import { searchFormRule } from "./rules";
import { mapGetters } from "vuex";
import { deepCloneClear,decomposeStringToJson } from "@/utils/tools";
...
export default {
  ...
  methods: {
    ...
    async getProcessList() {
      const { code, data, total } = await getProcessList({
        ...this.searchForm,
    ...
    formSearchMethod(self, reset) {
      ...
      let processType = decomposeStringToJson(formData.type,["processType","processTypeCode"]);
      this.searchForm = {
        ...formData,
        ...processDirection,
        ...processType,
