    ...
    <el-card class="box-card" shadow="never">
      ...
      <div class="user-form-table mt20">
        <el-button
          ...
          @click="$router.push({name:'pilotProduction',query:{type:'add'}})"
        >新增</el-button>
        <el-table
          class="mt10"
          :data="tableData"
        ...
        >
          <el-table-column prop="productionNumber" width="100" label="编号"></el-table-column>
          ...
          <el-table-column label="操作" align="center" width="180">
            <template v-slot="{row}">
              <common-operation
                :row="row"
                :deletUrl="deletUrl"
                :getPageList="getPilotProductionList"
                pageUrlName="pilotProduction"
...
<script>
/* eslint-disable */
import { getPilotProductionList, getPilotProductionDelete } from "@/service/pilot";
...
export default {
  ...
  data() {
    return {
      isUserType: false,
      deletUrl: getPilotProductionDelete,
      tableData: [],
      searchFormModel: {},
  ...
  methods: {
     ...
     * 查询项目申报列表
     */
    async getPilotProductionList() {
      const { code, data, total } = await getPilotProductionList({
        ...this.searchForm,
