<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix flex flex-center-between">
      <span>中试{{$titleObj[$route.query.type]}}</span>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      async getPilotTestDetails() {
        const {id, type} = this.$route.query;
        const {code, data} = await getPilotTestDetails({
      ...
      async postAdddProject(params, num) {
          //console.log("上传参数")
          //console.log(params)
        const {code, data} = await addPilotTestItem(params);
        if (code === 1) {
          if (num === 1) {
            this.$router.push({name: "pilotTest"});
          } else {
            this.$router.push({
              name: 'pilotTest',
      ...
      async updateProject(params, num) {
        const {code} = await updatePilotTestItem(params);
        if (code === 1) {
          this.loadingSave = false;
          this.loadingSubmit = false;
          if (num === 1) {
