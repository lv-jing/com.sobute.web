...
<template>
  <div>
    <common-title title="主要参与人员" class="md10" content="主要参与人员，可以通过拖动列表中最左侧图标进行相应的排序" />
    ...
    <div class="user-form-table">
      ...
      >
        ...
        <el-table-column prop="role" label="项目角色" width="200"></el-table-column>
        <!--<el-table-column label="项目负责人" width="100">-->
          <!--<template v-slot="{row}">-->
            <!--<span>{{row.queryFlag?'否':'是'}}</span>-->
          <!--</template>-->
        <!--</el-table-column>-->
        <!--<el-table-column label="项目审批人" width="100">-->
          <!--<template v-slot="{row}">-->
            <!--<span>{{row.approveFlag==1?'是':'否'}}</span>-->
          <!--</template>-->
        <!--</el-table-column>-->
        <el-table-column prop="empolyeeName" label="姓名" width="220"></el-table-column>
        <el-table-column prop="startDate" label="开始时间" width="220"></el-table-column>
        <el-table-column prop="endDate" label="结束时间" width="220"></el-table-column>
        ...
        <el-table-column prop="department" label="所属部门" width="300"></el-table-column>
        <!--<el-table-column prop="degree" label="最高学位" width="100"></el-table-column>-->
        <el-table-column prop="company" label="所在单位" width="445"></el-table-column>
        <!--<el-table-column-->
          <!--label="操作"-->
          <!--width="120"-->
          <!--fixed="right"-->
...
import { getHrListName } from "@/service/approval";
...
export default {
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { field: "queryFlag", title: "是否项目负责人" },
        { field: "approveFlag", title: "是否项目审批人" },
  ...
  computed: {
    ...mapState({
      ...
      userTypeCode: state => state.common.userTypeCode
    }),
  ...
  methods: {
    onChangeRoleSlect(val){
      const {code}=decomposeStringToJson(val,['name','code'])
      if(code==='2012101'){
        this.addEditFormModel.setValue('approveFlag','1')
      }else{
        this.addEditFormModel.setValue('approveFlag','0')
      }
    ...
    loopXLSXColumnData(columnList) {
      return new Promise(resolve => {
        ...
        if (!isBool) {
          resolve({ data: { ...item, uuid: this.$uuid() } });
        } else {
    ...
    handleAddList() {
      if (this.dateList && this.dateList.length > 0) {
        ...
        this.name = "";
        this.$nextTick(() => {
          this.addEditFormModel.hidden(
            this.userTypeCode == "1" ? true : false,
            "approveFlag"
          );
          this.addEditFormModel.resetFields();
          this.addEditFormModel.setValue({
