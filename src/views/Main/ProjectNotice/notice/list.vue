<template>
  <div class="bg-white">
    <el-card class="box-card" shadow="never">
      <div slot="header" class="clearfix flex flex-center-between">
        <span>公告管理</span>
        <!-- <el-button size="small" type="primary" icon="el-icon-plus" @click="showModel">{{title}}</el-button> -->
      </div>
      <div class="user-form-search">
        <form-create
          v-model="searchFormModel"
          @form-search-click="formSearchMethod"
          @form-reset-click="formResetMethod"
          :rule="searchFormRule"
          :option="searchOption"
        ></form-create>
      </div>
      <div class="user-form-table mt20">
        <el-button type="primary" v-if="!getAuthObj.flagAdd" size="small" @click="addDeclaration"
                   icon="iconfont sbt_add">新增
        </el-button>
        <el-table
          class="mt10"
          :data="tableData"
          stripe highlight-current-row
          style="width: 100%"
          :header-cell-style="{background:'#f3f3f3'}"
          size="small"
        >
          <el-table-column prop="title" label="公告标题">
            
            <template v-slot={row}>
              <a @click="previewNotice(row)" href="javascript:;">{{row.title}}</a>
            </template>


          </el-table-column>
         
          <el-table-column prop="status" width="100" label="状态">
            <template v-slot={row}>
              {{['停用','启用'][row.status]}}
            </template>
          </el-table-column>
          <el-table-column prop="startDate" width="200" label="有效期限">
            <template slot-scope="scope">
              <p>{{scope.row.startDate}}至{{scope.row.endDate}}</p>
            </template>
          </el-table-column>
          <el-table-column prop="creationDate" width="150" label="创建时间"></el-table-column>
          <el-table-column prop="createdByName" width="100"  label="创建人"></el-table-column>
          <el-table-column prop  label="操作"   align="center" width="150">
            <template v-slot="{row,$index}">
              <el-button type="text" @click="$router.push({name: 'notice', query: {type: 'edit',id:row.guid}})">编辑</el-button>
                <el-popconfirm
                  title="确认删除该条记录吗?"
                  @onConfirm="handleDeleteClick(row,$index)"
                >
                  <el-button class="ml10" slot="reference" type="text" size="mini">删除</el-button>
                </el-popconfirm>
            </template>
          </el-table-column>
        </el-table>
        <common-pagination v-on:handler-currentPage="handlerCurrentPage" :list="searchForm"></common-pagination>
      </div>
    </el-card>







  </div>
</template>

<script>
  /* eslint-disable */
  import {searchFormRule} from "./rules";

  import {exportFile} from "@/utils/tools";
  import {
    getProjectList,
    deleteProjectItem,
  } from "@/service/project";
  import {getDownLoadWord,allNarmalInterface} from '@/service/common'
  import {mapGetters} from 'vuex'
  import {getDictionaryTypeName} from "@/service/system";
  import {transformValueAndLabel,deepCloneClear} from "@/utils/tools";

  export default {
    name: "apply",
    data() {
      return {
        previewShow:false,
        details:{},
        downUrl: getDownLoadWord,// 下载地址
        deletUrl: deleteProjectItem,// 删除地址
        tableData: [],
        searchFormModel: {},
        searchFormRule: searchFormRule,
        searchOption: {
          resetBtn: false,
          submitBtn: false,
          form: {
            size: "small"
          }
        },
        searchForm: {
          pageNum: 1,
          pageSize: 10,
          total: 0
        }
      };
    },
    computed: {
      ...mapGetters(['getAuthObj'])
    },
    created() {
      this.$store.dispatch("setHeight", 0);
      this.getDeclarationList();
    },
    mounted() {
      this.initData();
    },
    methods: {
   /**
     * 预览
     */
    async previewNotice(item) {
      this.$router.push({name:'default',query:{id:item.guid}})
    },
      /**
       * 新增按钮事件
       */
      addDeclaration() {
        this.$router.push({name: "notice", query: {type: "add"}});
      },
      /**
       * 查询
       */
      formSearchMethod(self) {
        let formData = self.$f.formData();
        this.searchForm = {
          ...formData,
          pageNum: 1,
          pageSize: 10,
          total: 0
        };
        this.getDeclarationList();
      },
      /**
       * 重置
       */
      formResetMethod(self) {
        self.$f.resetFields();
        this.initData();
        this.formSearchMethod(self);
      },
      /**
       * 初始化表单数据
       */
      initData() {
        const formData = this.searchFormModel.formData();
        let clearFormData = deepCloneClear(formData);
        this.searchFormModel.setValue(clearFormData);
      },
        /**
       * 分页查询
       */
      handlerCurrentPage(list) {
        this.searchForm = {
          ...this.searchForm,
          ...list
        };
        
        this.getDeclarationList();
      },
      /**
       * 查询列表
       */
      async getDeclarationList() {
        console.log(  this.searchForm,)
        const {code, data, total} = await allNarmalInterface(
          {params:this.searchForm},
          "service/base/sysNotice/list",
          'get'
        );
        if (code === 1) {
          this.tableData = data;
          this.searchForm.total = total;
        }
      },
        /**
     * 删除
     */
    async handleDeleteClick({ guid }, index) {
      const { code, data } = await allNarmalInterface({  },`service/base/sysNotice/${guid}`,'delete');
      if (code === 1) {
        this.tableData.splice(index, 1);
        this.$message({
          type: "success",
          message: "操作成功"
        });
      }
    },
    }
  };
</script>

<style lang="scss" scoped>
.notice-summary {
  background: #f2f2f2;
  border-radius: 10px;
  padding: 10px;
  line-height: 22px;
}
</style>