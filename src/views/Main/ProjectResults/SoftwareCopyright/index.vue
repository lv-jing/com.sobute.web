<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix flex flex-center-between">
      <span>软件著作权{{$titleObj[$route.query.type]}}</span>
    </div>
    <el-tabs type="card">
...
export default {
  ...
  data() {
    return {
      ...
      type: {
        type: "软件著作权", // 项目类型
        typeCode: "2014105" // 项目类型code
      },
      uploadParams: {
        bizType: "软件著作权", //上传项目类型
        bizTypeCode: "2013113" //附件类型编码
      },
      uploadParamsOne: {
        bizType: "软件著作新建", //业务分类
        bizTypeCode: "2101000", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      uploadParamsTwo: {
        bizType: "软件著作审批", //业务分类
        bizTypeCode: "2101100", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      attachedTable: [],
  ...
  methods: {
    ...
    async getProjectDetail() {
      ...
      const { code, data } = await getSoftwareDetail({
    ...
    async postAddProject(params, num) {
      const { code, data } = await postAddSoftware(params);
      if (code === 1) {
        if (num === 1) {
    ...
    async updateProject(params, num) {
      const { code, data } = await updateSoftware(params);
      if (code === 1) {
        this.loadingSave = false;
        this.loadingSubmit = false;
        if (num === 1) {
