<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix flex flex-center-between">
      <span>学术专著{{$titleObj[$route.query.type]}}</span>
    </div>
    <el-tabs type="card">
...
export default {
  ...
  data() {
    return {
      ...
      type: {
        type: "学术专著", // 项目类型
        typeCode: "2014104" // 项目类型code
      },
      uploadParams: {
        bizType: "学术专著", //上传项目类型
        bizTypeCode: "2013111" //附件类型编码
      },
      uploadParamsOne: {
        bizType: "已审批阶段", //业务分类
        bizTypeCode: "2102500", //业务类型编码
        bizSubType: "",
  ...
  methods: {
    ...
    init() {
      ...
      const { statusCode } = this.rpmRtAchievement;
    ...
    async getProjectDetail() {
      ...
      const { code, data } = await getLearningDetail({
    ...
    async postAddProject(params, num) {
      const { code, data } = await postAddLearning(params);
      if (code === 1) {
        if (num === 1) {
    ...
    async updateProject(params, num) {
      const { code, data } = await updateLearning(params);
      if (code == 1) {
        if (num === 1) {
          this.$router.push({ name: "results" });
        }
