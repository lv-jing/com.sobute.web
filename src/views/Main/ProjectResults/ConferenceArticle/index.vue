<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix flex flex-center-between">
      <span>会议论文{{$titleObj[$route.query.type]}}</span>
    </div>
    <el-tabs type="card">
...
export default {
  ...
  data() {
    return {
      ...
      type: {
        type: "会议论文", // 项目类型
        typeCode: "2014108" // 项目类型code
      },
      uploadParams: {
        bizType: "会议论文", //业务分类
        bizTypeCode: "2013104" //附件业务类型编码
      },
      uploadParamsOne: {
        bizType: "新建阶段", //业务分类
        bizTypeCode: "2101800", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      uploadParamsTwo: {
        bizType: "已审批阶段", //业务分类
        bizTypeCode: "2101900", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      attachedTable: [],
  ...
  methods: {
    ...
    init() {
      this.addEditFormModel.reload([
        ...
        ...commonRule(this.userInfo.type,130)
      ]);
    ...
    async getProjectDetail() {
      ...
      const { code, data } = await getMeetingDetail({
      ...
      if (code === 1) {
        this.$nextTick(() => {
          ...
          rpmRtAchievement.report = rpmRtAchievement.report
            ? rpmRtAchievement.report + "⭐" + rpmRtAchievement.reportCode
            : "";
...
let rpmRtAchievement={};
      ...
      this.addEditFormModel.submit(formData => {
        ...
        let report = decomposeStringToJson(formData.report, [
          "report",
          "reportCode"
        ]);
        ...
        formData.confirmDate =
          (formData.confirmDate && formData.confirmDate + "-01") || "";
         rpmRtAchievement = {
          ...formData,
          ...status,
          ...this.type,
    ...
    async postAddProject(params, num) {
      const { code, data } = await postAddMeeting(params);
      if (code === 1) {
        if (num === 1) {
    ...
    async updateProject(params, num) {
      const { code, data } = await updateMeeting(params);
      if (code === 1) {
        this.loadingSave = false;
        this.loadingSubmit = false;
        if (num === 1) {
