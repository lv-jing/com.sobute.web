<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix flex flex-center-between">
      <span>应用证明{{$titleObj[$route.query.type]}}</span>
    </div>
    <el-tabs type="card">
...
import { postAddApp, getAppDetail, updateApp } from "@/service/results";
import {
...
export default {
  ...
  data() {
    return {
      ...
      type: {
        type: "应用证明", // 项目类型
        typeCode: "2014110" // 项目类型code
      },
      uploadParams: {
        bizType: "应用证明", //上传项目类型
        bizTypeCode: "2013112" //附件类型编码
      },
      uploadParamsOne: {
        bizType: "已审批阶段", //业务分类
        bizTypeCode: "2102400", //业务类型编码
        bizSubType: "",
  ...
  methods: {
    ...
    async getProjectDetail() {
      ...
      const { code, data } = await getAppDetail({
      ...
      if (code === 1) {
        this.$nextTick(() => {
          ...
          if (
            rpmRtAchievement.statusCode === "2005104" ||
            rpmRtAchievement.statusCode === "2005107"
          ) {
    ...
    async postAddProject(params, num) {
      const { code, data } = await postAddApp(params);
      if (code === 1) {
        if (num === 1) {
    ...
    async updateProject(params, num) {
      const { code, data } = await updateApp(params);
      if (code === 1) {
        this.loadingSave = false;
        this.loadingSubmit = false;
        if (num === 1) {
    ...
    handleSubmit() {
      this.addEditFormModel.validate(valid => {
        if (valid) {
          if (this.uploadShow && !getStatus(this.uploadTableData)) {
            return;
          }
