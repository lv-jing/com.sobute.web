<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix flex flex-center-between">
      <span>期刊论文{{$titleObj[$route.query.type]}}</span>
    </div>
    <el-tabs type="card">
...
export default {
  ...
  data() {
    return {
      ...
      type: {
        type: "期刊论文", // 项目类型
        typeCode: "2014107" // 项目类型code
      },
      uploadParams: {
        bizType: "期刊论文", //上传项目类型
        bizTypeCode: "2013103" //附件类型编码
      },
      uploadParamsOne: {
        bizType: "新建阶段", //业务分类
        bizTypeCode: "2101600", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      uploadParamsTwo: {
        bizType: "已审批阶段", //业务分类
        bizTypeCode: "2101700", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      attachedTable: [],
  ...
  methods: {
    ...
    async getProjectDetail() {
      ...
      const { code, data } = await getPeriodicalDetail({
    ...
    async postAddProject(params, num) {
      const { code, data } = await postAddPeriodical(params);
      if (code === 1) {
        if (num === 1) {
    ...
    async updateProject(params, num) {
      const { code, data } = await updatePeriodical(params);
      if (code === 1) {
        if (num === 1) {
          this.$router.push({ name: "results" });
        }
