<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix flex flex-center-between">
      <span>标准规范{{$titleObj[$route.query.type]}}</span>
    </div>
    <el-tabs type="card">
...
export default {
  ...
  data() {
    return {
      ...
      type: {
        type: "标准规范", // 项目类型
        typeCode: "2014103" // 项目类型code
      },
      uploadParams: {
        bizType: "标准规范", //上传项目类型
        bizTypeCode: "2013110" //附件类型编码
      },
      uploadParamsOne: {
        bizType: "新建阶段", //业务分类
        bizTypeCode: "2102200", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      uploadParamsTwo: {
        bizType: "已审批阶段", //业务分类
        bizTypeCode: "2102300", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      attachedTable: [],
  ...
  methods: {
    ...
    async getProjectDetail() {
      ...
      const { code, data } = await getStandardDetail({
        id: this.$route.query.id
      });
      if (code === 1) {
        this.$nextTick(() => {
          let rpmRtAchievement = data.rpmRtAchievement || {};
          rpmRtAchievement.source =
            rpmRtAchievement.source &&
            rpmRtAchievement.source + "⭐" + rpmRtAchievement.sourceCode;
          if (
            type === "view" ||
            type === "auditing" ||
    ...
    getParams(val) {
      ...
      this.addEditFormModel.submit(formData => {
        ...
        let source = decomposeStringToJson(formData.source && formData.source, [
          "source",
          "sourceCode"
        ]);
    ...
    async postAddProject(params, num) {
      const { code, data } = await postAddStandard(params);
      if (code === 1) {
        if (num === 1) {
    ...
    async updateProject(params, num) {
      const { code, data } = await updateStandard(params);
      if (code === 1) {
        this.loadingSave = false;
        this.loadingSubmit = false;
        if (num === 1) {
