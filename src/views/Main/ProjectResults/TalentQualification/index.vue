<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix flex flex-center-between">
      <span>人才/资质/奖励/专利奖{{$titleObj[$route.query.type]}}</span>
    </div>
    <el-tabs type="card">
      <el-tab-pane label="基本信息">
        ...
        <common-required-upload
          ...
          :uploadParamsTwo="uploadParamsTwo"
...
export default {
  ...
  data() {
    return {
      ...
      type: {
        type: "人才/资质/奖励/专利奖", // 项目类型
        typeCode: "2014106" // 项目类型code
      },
      uploadParams: {
        bizType: "人才/资质/奖励/专利奖", //上传项目类型
        bizTypeCode: "2013114" //附件类型编码
      },
      uploadParamsOne: {
        bizType: "新建阶段", //业务分类
        bizTypeCode: "2102600", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      uploadParamsTwo: {
        bizType: "已审核阶段", //业务分类
        bizTypeCode: "2102700", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      attachedTable: [],
  ...
  methods: {
    ...
    init() {
      this.addEditFormModel.reload([
        ...
        ...commonRule(this.userInfo.type,120)
      ]);
    ...
    async getProjectDetail() {
      ...
      let { statusCode } = this.rpmRtAchievement;
      const { code, data } = await getRewardDetail({
      ...
      if (code === 1) {
        this.$nextTick(() => {
          ...
          let rpmRtAchievement = data.rpmRtAchievement || {};
            rpmRtAchievement.refRtAchievementId=rpmRtAchievement.refRtAchievementId&&rpmRtAchievement.refRtAchievementId.split(',')||[];
            rpmRtAchievement.refRtPatentId=rpmRtAchievement.refRtPatentId&&rpmRtAchievement.refRtPatentId.split(',')||[]
    ...
    getParams(val) {
      ...
      this.addEditFormModel.submit(formData => {
        ...
        formData.refRtPatentId=formData.refRtPatentId?.toString()||"";
        formData.refRtAchievementId=formData.refRtAchievementId?.toString()||"";
    ...
    async postAddProject(params, num) {
      ...
      const { code, data } = await postAddReward(params);
      if (code === 1) {
        if (num === 1) {
    ...
    async updateProject(params, num) {
      const { code, data } = await updateReward(params);
      if (code === 1) {
        this.loadingSave = false;
        this.loadingSubmit = false;
        if (num === 1) {
