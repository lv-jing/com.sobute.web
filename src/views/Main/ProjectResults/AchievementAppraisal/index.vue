<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix flex flex-center-between">
      <span>成果鉴定{{$titleObj[$route.query.type]}}</span>
    </div>
    <el-tabs type="card">
...
export default {
  ...
  data() {
    return {
      ...
      type: {
        type: "成果鉴定", // 项目类型
        typeCode: "2014109" // 项目类型code
      },
      uploadParams: {
        bizType: "成果鉴定", //上传类型
        bizTypeCode: "2013105" //附件类型编码
      },
      uploadParamsOne: {
        bizType: "新建阶段", //业务分类
        bizTypeCode: "2102000", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      uploadParamsTwo: {
        bizType: "已审批阶段", //业务分类
        bizTypeCode: "2102100", //业务类型编码
        bizSubType: "",
        bizSubTypeCode: "" //业务类型编码
      },
      attachedTable: [],
  ...
  methods: {
    ...
    async getProjectDetail() {
      ...
      const { code, data } = await getVerifiedDetail({
...
let rpmRtAchievement={};
    ...
    async postAddProject(params, num) {
      const { code, data } = await postAddVerified(params);
      if (code === 1) {
        if (num === 1) {
    ...
    async updateProject(params, num) {
      const { code, data } = await updateVerified(params);
      if (code == 1) {
        this.loadingSave = false;
        this.loadingSubmit = false;
        if (num === 1) {
