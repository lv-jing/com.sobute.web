...
export default {
  ...
  watch: {
    // selectRows(val) {
    //   this.approvePserson = val;
    // },
    processParam(val) {
      const { id, type, codeType } = this.$route.query;
      if (
        ["detail", "view", "auditing"].includes(type) ||
        this.userShow ||
        codeType === "2005104"
      ) {
        this.getApprovedHistoryList(val);
      }
    }
  },
  mounted() {
    let { type: routeType, id } = this.$route.query;
